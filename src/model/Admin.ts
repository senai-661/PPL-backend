...
export class Admin {
    ...
    static async buscarPorEmail(email: string): Promise<Admin | null> {
        try {
            const res = await database.query(`SELECT * FROM administrador WHERE email = $1`, [email]);
    ...
    try {
        const res = await database.query(`SELECT id_admin, nome, email, senha FROM administrador;`);
        
        // Retorna um array de objetos Admin
        return res.rows.map(a => new Admin(a.id_admin, a.nome, a.email, a.senha));
    } catch (error) {
